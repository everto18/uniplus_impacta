# Security Module - Security Groups, IAM Roles, WAF
# Implements Zero Trust networking and least privilege access
...
resource "aws_security_group" "ecs_tasks" {
  ...
  ingress {
    description     = "Video API port from ALB"
    from_port       = 3000
    to_port         = 3000
    protocol        = "tcp"
    security_groups = [aws_security_group.alb.id]
...
#------------------------------------------------------------------------------
# IAM Role - ECS Task Role (for application)
#------------------------------------------------------------------------------
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:s3:::${var.project_name}-*-raw-videos-*/*"
      },
      {
        Effect = "Allow"
        Action = [
        ...
        Resource = "arn:aws:s3:::${var.project_name}-*-processed-videos-*/*"
      }
    ]
  })
}
