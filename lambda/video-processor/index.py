...
Flow: S3 Upload → EventBridge → SQS → This Lambda → MediaConvert
"""
...
def handler(event, context):
    """
    Process SQS messages and create MediaConvert jobs.
    
    Each message contains:
    - bucket: S3 bucket name
    - key: S3 object key (video file path)
    - size: File size
    - requestId: Original S3 request ID
    ...
    for record in event.get('Records', []):
        try:
            ...
            raise  # Re-raise to trigger retry via SQS
    
    return {
        'statusCode': 200,
        'body': json.dumps({
            'message': 'Video processing jobs created',
            'results': results
        })
    }
